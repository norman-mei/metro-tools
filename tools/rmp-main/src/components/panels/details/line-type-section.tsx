 ...
 */
const isLinePathAndStyleDisabled = (pathType: LinePathType, styleType: LineStyleType) => {
    // This must be placed first as legacy simple path combos are handled specially.
...
export default function LineTypeSection() {
    ...
    const disabledLinePathOptions = Object.values(LinePathType).filter(linePathType =>
        isLinePathAndStyleDisabled(linePathType, currentLineStyleType)
    );
    const disabledLineStyleOptions = Object.values(LineStyleType).filter(lineStyleType =>
        isLinePathAndStyleDisabled(currentLinePathType, lineStyleType)
    );
