...
import { Id, NodeId, RuntimeMode, StnId } from '../constants/constants';
import { LinePathType } from '../constants/lines';
import { MiscNodeType } from '../constants/nodes';
import { StationAttributes, StationType } from '../constants/stations';
import { useRootDispatch, useRootSelector } from '../redux';
import { setKeepLastPath, setSnapLines } from '../redux/app/app-slice';
...
import { makeParallelIndex, NonSimpleLinePathAttributes } from '../util/parallel';
...
const SvgWrapper = () => {
    ...
    const {
        preference: { gridLines, snapLines, predictNextNode, autoParallel, autoChangeStationType, keepLastPath },
    } = useRootSelector(state => state.app);
    const { svgViewBoxZoom, svgViewBoxMin } = useRootSelector(state => state.param);
    const { selected, active, isDetailsOpen, mode, lastTool, theme } = useRootSelector(state => state.runtime);

    const size = useWindowSize();
    const { height, width } = getCanvasSize(size);

    const isMasterDisabled = false;
    const isParallelDisabled = !autoParallel;
