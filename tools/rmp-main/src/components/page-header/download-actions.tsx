...
import { exportProjectXlsx } from '../../util/xlsx-project';
...
export default function DownloadActions() {
    ...
    const handleDownloadXlsx = async () => {
        try {
            const blob = await exportProjectXlsx(graph.current, {
                svgViewBoxZoom: param.svgViewBoxZoom,
                svgViewBoxMin: param.svgViewBoxMin,
            });
            downloadBlobAs(`RMP_${new Date().valueOf()}.xlsx`, blob);
        } catch (error) {
            dispatch(setGlobalAlert({ status: 'error', message: (error as Error).message || t('header.open.unknownError') }));
        }
    };
    ...
    return (
        <Menu id="download">
            ...
            <MenuList>
                ...
                <MenuItem icon={<MdSaveAs />} onClick={handleDownloadXlsx}>
                    Export project (XLSX)
                </MenuItem>
                        ...
                        {format === 'png' && resvgScaleOptions.includes(scale) && !isTauri && (
                            <Alert status="error" mt="4">
                                ...
                                <Box>
                                    ...
                                    <AlertDescription>
                                        Reduce the scale, or use the desktop app for large exports:{' '}
                                        <Link
                                            color="teal.500"
