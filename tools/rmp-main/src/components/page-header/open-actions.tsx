...
import { parseProjectXlsx } from '../../util/xlsx-project';
...
export default function OpenActions() {
    ...
    const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!isJson && !isXlsx) {
            dispatch(setGlobalAlert({ status: 'error', message: 'Please upload a JSON or XLSX file.' }));
            logger.error('OpenActions.handleUpload():: Invalid file type! Only JSON/XLSX formats are accepted.');
        } else {
            try {
                if (!file) {
                ...
                let version = 0;
                if (isJson) {
                    paramStr = await readFileAsText(file);
                    version = parseVersionFromSave(paramStr);
                } else {
                    const save = await parseProjectXlsx(file);
                    paramStr = JSON.stringify(save);
