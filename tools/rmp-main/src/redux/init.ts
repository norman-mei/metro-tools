...
import { setState } from './account/account-slice';
import {
...
export const initStore = async (store: RootStore) => {
    ...
    // Keep Redux preference in sync with current runtime theme mode.
    store.dispatch(setThemeMode(rmgRuntime.getColourMode()));
    ...
    if ('preference' in appState) {
        ...
        if ('keepLastPath' in appState.preference) store.dispatch(setKeepLastPath(appState.preference.keepLastPath));
        if (
            'themeMode' in appState.preference &&
            ['light', 'dark', 'system'].includes(appState.preference.themeMode)
        ) {
            const themeMode = appState.preference.themeMode as 'light' | 'dark' | 'system';
            store.dispatch(setThemeMode(themeMode));
            rmgRuntime.setColourMode(themeMode);
        }
